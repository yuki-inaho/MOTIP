...
- **代表的なコマンド:**
  ...
  - AppleMOTS train/0000の小規模overfit切り分けでは、GT mean track length 20.93に対し、AppleMOTS pretrain raw MOTIPは4.18。MOTIP bbox streamをOptuna center-distance retrackすると34.51まで伸びたが、そのretrack教師でMOTIPをjoint fine-tuneしてもraw出力は2.37、DETR freeze + ID-only overfitでも1.84に留まった。一方でID-only checkpointのonly-DETR bboxへ同じretrackを当てると28.50まで戻る。結論として、bbox/空間運動から長trackletは作れるが、現状のMOTIP ID decoder/RuntimeTrackerはtrain subsetでもretrack教師IDを再現できていない。果実の外観特徴が似すぎてID分離信号が弱い可能性が高く、実用出力は `--only-detr` bbox stream + tuned retracker を優先する。関連config/target: `configs/finetune_applemots_overfit_train0000_retrack.yaml`, `configs/finetune_applemots_overfit_train0000_retrack_idonly.yaml`, `just build-applemots-overfit-retrack-train0000`, `just train-applemots-overfit-retrack-idonly`, `just infer-applemots-overfit-retrack-idonly-train0000`。
